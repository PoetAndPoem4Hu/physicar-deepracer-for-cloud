...
if [[ -z "${OPT_FORCE}" ]]; then
    if [[ -n "${OPT_LOCAL}" ]]; then
        echo "Ready to upload car model to local s3://${DR_LOCAL_S3_BUCKET}/${DR_UPLOAD_S3_PREFIX}/physical-car-model/last.tar.gz"
    else
        echo "Ready to upload car model to remote s3://${DR_UPLOAD_S3_BUCKET}/${DR_UPLOAD_S3_PREFIX}/physical-car-model/last.tar.gz"
    fi
...
#upload to s3
if [[ -n "${OPT_LOCAL}" ]]; then
    aws ${DR_LOCAL_PROFILE_ENDPOINT_URL} s3 cp carfile.tar.gz s3://${DR_LOCAL_S3_BUCKET}/${DR_UPLOAD_S3_PREFIX}/physical-car-model/last.tar.gz
else
