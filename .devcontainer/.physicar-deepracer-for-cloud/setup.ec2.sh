...
######## systemd 서비스 등록 - entrypoint
USERNAME=${SUDO_USER:-$(id -un)}
HOMEDIR=$(getent passwd "$USERNAME" | cut -d: -f6)

sudo chmod +x "$HOMEDIR/.physicar-deepracer-for-cloud/entrypoint.ec2.sh"

sudo tee /etc/systemd/system/physicar-deepracer-for-cloud-entrypoint@.service >/dev/null <<'UNIT'
[Unit]
...
ExecStart=/usr/bin/tmux new-session -d -s physicar-deepracer-for-cloud-entrypoint "/home/%i/.physicar-deepracer-for-cloud/entrypoint.ec2.sh"
RemainAfterExit=yes
Restart=on-failure
...
sudo systemctl enable --now "physicar-deepracer-for-cloud-entrypoint@${USERNAME}.service"
